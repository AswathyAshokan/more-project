	...
	if r.Method == "POST" {
		...
		case true:
			...
			case true:
				...
				if err := smtp.SendMail("smtpout.secureserver.net:465", smtp.PlainAuth("", "anitha@ceemedia.in", " pass2017@", "smtpout.secureserver.net"), from, []string{to}, []byte(message)); err != nil {
				log.Println(err)
				}
